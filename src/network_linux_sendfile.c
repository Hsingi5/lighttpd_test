...
int network_write_chunkqueue_linuxsendfile(server *srv, connection *con, int fd, chunkqueue *cq, off_t max_bytes) {
	chunk *c;
	// printf("network_write_chunkqueue_linuxsendfile\n");
	for(c = cq->first; (max_bytes > 0) && (NULL != c); c = c->next) {
		int chunk_finished = 0;

		switch(c->type) {
		case MEM_CHUNK: {
			//wxy_mem_chunk write for decide if in mem
			// printf("mem_chunk. search wxy_mem_chunk to delete\n");
			char * offset;
			off_t toSend;
			ssize_t r;
		...
		case FILE_CHUNK: {
			...
			//wxy_mem_chunk write for decide if in mem
			// printf("file_chunk. search wxy_mem_chunk to delete\n");
